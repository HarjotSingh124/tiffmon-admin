...
export default function SubscriptionCustomers() {
  ...
  useEffect(() => {
    const q = query(collection(db, "customerSubscriptions"), orderBy("createdAt", "desc"));
  ...
  async function handleSubmit(e) {
    ...
    if (!cleanSubscriberId) return alert("Subscriber ID is required.");
    if (!cleanPassword) return alert("Password is required.");
  ...
  async function updateCredentials(customer) {
    const subscriberId = prompt("Enter Subscriber ID:", customer.subscriberId || "");
    if (!subscriberId) return;
    ...
    const duplicateSnapshot = await getDocs(duplicateQuery);
    const duplicateExists = duplicateSnapshot.docs.some((item) => item.id !== customer.id);
  ...
  const stats = useMemo(() => {
    return {
      total: customers.length,
      active: customers.filter((c) => (c.subscriptionStatus || "active") === "active").length,
      paused: customers.filter((c) => c.subscriptionStatus === "paused").length,
      paid: customers.filter((c) => (c.paymentStatus || "unpaid") === "paid").length,
    };
  }, [customers]);
  ...
  const filteredCustomers = useMemo(() => {
    ...
    if (statusFilter !== "all") {
      result = result.filter((customer) => (customer.subscriptionStatus || "active") === statusFilter);
    }

    if (paymentFilter !== "all") {
      result = result.filter((customer) => (customer.paymentStatus || "unpaid") === paymentFilter);
    }
    ...
    if (searchTerm.trim()) {
      ...
      result = result.filter((customer) => {
        return (
          ...
          (customer.subscriberId || "").toLowerCase().includes(q)
        );
      });
    }
  ...
  return (
    <AdminLayout title="Subscribed Customers" subtitle="Track subscribers, meals, and login credentials">
      <section className="panel" style={{ marginBottom: "18px" }}>
        <div className="panel-head">
          <h2>Add Subscriber Manually</h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="grid-form">
          ...
          <input name="planName" placeholder="Plan Name *" value={form.planName} onChange={handleChange} required />
          ...
          <input name="totalMeals" type="number" placeholder="Total Meals *" value={form.totalMeals} onChange={handleChange} required />
          <input name="startDate" type="date" value={form.startDate} onChange={handleChange} required />
          <input name="subscriberId" placeholder="Subscriber ID *" value={form.subscriberId} onChange={handleChange} required />
          ...
          <div className="form-actions">
            <button type="submit" className="primary-btn">Add Subscriber</button>
      ...
      <section className="stats-grid">
        <div className="stat-card"><div className="stat-icon">👤</div><div><p>Total Subscribers</p><h3>{stats.total}</h3></div></div>
        <div className="stat-card"><div className="stat-icon">✅</div><div><p>Active</p><h3>{stats.active}</h3></div></div>
        <div className="stat-card"><div className="stat-icon">⏸️</div><div><p>Paused</p><h3>{stats.paused}</h3></div></div>
        <div className="stat-card"><div className="stat-icon">💳</div><div><p>Paid</p><h3>{stats.paid}</h3></div></div>
      </section>
      ...
      <section className="panel">
        ...
        <div className="subscribers-toolbar">
          ...
          <div className="subscribers-filters">
            ...
            <select className="sheet-select" value={paymentFilter} onChange={(e) => setPaymentFilter(e.target.value)}>
              <option value="all">All Payments</option>
              <option value="paid">Paid</option>
              <option value="unpaid">Unpaid</option>
              <option value="overdue">Overdue</option>
          ...
          <table className="subscribers-sheet">
            ...
            <tbody>
              {filteredCustomers.map((customer) => {
                const totalMeals = Number(customer.totalMeals || 0);
                const mealsTaken = Number(customer.mealsTaken || 0);
                const remainingMeals = totalMeals > 0 ? Math.max(totalMeals - mealsTaken, 0) : 0;

                return (
                  <tr key={customer.id}>
                    <td><div className="sheet-name"><strong>{customer.customerName || "Unnamed"}</strong><span>{customer.addressLine1 || ""}</span></div></td>
                    <td>{customer.phone || "N/A"}</td>
                    <td><div className="sheet-name"><strong>{customer.planName || "No Plan"}</strong><span>₹{customer.price || 0} • {customer.period || ""}</span></div></td>
                    <td>{formatDate(customer.startDate)}</td>
                    <td><span className={`order-status status-${customer.subscriptionStatus || "active"}`}>{customer.subscriptionStatus || "active"}</span></td>
                    <td><span className={`order-status payment-${customer.paymentStatus || "unpaid"}`}>{customer.paymentStatus || "unpaid"}</span></td>
                    <td>{totalMeals}</td>
                    <td>
                      <div className="meal-counter">
                        <button className="meal-mini-btn" onClick={() => changeMealsTaken(customer, -1)}>−</button>
                        <span>{mealsTaken}</span>
                        <button className="meal-mini-btn" onClick={() => changeMealsTaken(customer, 1)}>+</button>
                      </div>
                    </td>
                    ...
                    <td>
                      <div className="sheet-actions">
                        <button className="mini-btn" onClick={() => updateCredentials(customer)}>Set Login</button>
                        <button className="mini-btn primary-btn" onClick={() => updateSubscriptionStatus(customer.id, "active")}>Active</button>
                        <button className="mini-btn secondary-btn" onClick={() => updateSubscriptionStatus(customer.id, "paused")}>Pause</button>
                        <button className="mini-btn danger-btn" onClick={() => updateSubscriptionStatus(customer.id, "cancelled")}>Cancel</button>
                        <button className="mini-btn success-btn" onClick={() => updatePaymentStatus(customer.id, "paid")}>Paid</button>
                      </div>
                    </td>
                  </tr>
                );
              ...
              {filteredCustomers.length === 0 && (
                <tr>
                  <td colSpan="13">
                    <div className="empty-state">No subscribed customers found.</div>
                  </td>
                </tr>
              )}
            </tbody>
